...
export default function BackofficeLogin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/dashboard/backoffice-login', {
        method: 'POST',
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
      <section className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl">
        ...
        <p className="mb-6 text-sm text-slate-400">Acesso administrativo da plataforma. Esta área possui autenticação independente do ambiente dos clientes.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 font-semibold text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-60"><LockKeyhole size={18} /> {loading ? 'Autenticando...' : 'Entrar no Backoffice'}</button>
        </form>
