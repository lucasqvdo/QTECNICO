...
async function syncItem(item:QueueItem){
  if(item.type==='order_update'){const saved=await api.updateOrder(item.orderId,item.order);await cacheOrders([saved]);return;}
  if(item.type==='attendance_photo'){if(!item.attendanceId)throw new Error('Atendimento da foto não identificado.');const file=asFile(item);const uploaded=await api.uploadPhoto(file,'attendances');await api.registerAttendancePhoto(item.orderId,item.attendanceId,{key:uploaded.key,url:uploaded.url,name:item.fileName||file.name});return;}
  if(item.type==='signature_upload'){const file=asFile(item);const uploaded=await api.uploadPhoto(file,'signatures');const saved=await api.updateOrder(item.orderId,{clientSignature:uploaded.url,clientSignatureKey:uploaded.key,status:'completed'});await cacheOrders([saved]);return;}
  throw new Error('Operação offline desconhecida.');
}
export async function syncOfflineQueue():Promise<void>{if(running||!navigator.onLine)return;running=true;state='syncing';emit();try{const queue=(await getQueue()).sort((a,b)=>a.createdAt-b.createdAt);for(const item of queue){if(!navigator.onLine)break;try{await syncItem(item);await removeQueueItem(item.id);}catch(error){await updateQueueFailure(item.id,error instanceof Error?error.message:'Falha na sincronização');state='error';emit();break;}}if(state!=='error'){await markServerSync();state='idle';}emit();}finally{running=false;}}
