...
import { BarChart3, CalendarDays, CheckCircle2, ClipboardList, DollarSign, LogOut, Menu, Users, Wrench, X } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    const revenue = filteredOrders.reduce((s, o) => s + o.clientValue, 0);
    const paid = filteredOrders.reduce((s, o) => s + (o.payments?.length ? o.payments.filter((p) => p.status === 'paid').reduce((a, p) => a + p.amount, 0) : o.paymentStatus === 'paid' ? (o.paidAmount ?? o.clientValue) : 0), 0);
    const costs = filteredOrders.reduce((s, o) => s + o.expenses.reduce((a, e) => a + e.amount, 0), 0);
  ...
  const statusData = [{ name: 'Pendentes', value: stats.pending }, { name: 'Em andamento', value: stats.progress }, { name: 'Concluídas', value: stats.completed }, { name: 'Canceladas', value: filteredOrders.filter((o) => o.status === 'cancelled').length }].filter((x) => x.value > 0);
  const monthData = useMemo(() => { const map = new Map<string, { name: string; valor: number }>(); filteredOrders.forEach((o) => { const d = new Date(o.date + 'T12:00:00'); const key = `${d.getFullYear()}-${d.getMonth()}`; const v = map.get(key) ?? { name: d.toLocaleDateString('pt-BR', { month: 'short' }).replace('.', ''), valor: 0 }; v.valor += o.clientValue; map.set(key, v); }); return [...map.entries()].sort(([a], [b]) => a.localeCompare(b)).slice(-6).map(([, v]) => v); }, [filteredOrders]);
