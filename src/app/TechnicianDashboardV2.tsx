...
import { startOfflineSync } from './offlineSync';
...
const dateTimeLabel = (value: string | null | undefined) => { if (!value) return '—'; const date = new Date(value); return Number.isNaN(date.getTime()) ? '—' : date.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }); };
const openNavigation = (address: string | null | undefined) => { const destination = encodeURIComponent(String(address || '').trim()); if (destination) window.location.href = `https://www.google.com/maps/dir/?api=1&destination=${destination}`; };
function Stat({ label, value, icon: Icon, detail }: { label: string; value: number; icon: typeof Wrench; detail: string }) { return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold tracking-tight text-slate-900">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></div><div className="rounded-xl bg-cyan-50 p-3 text-cyan-600"><Icon size={21} /></div></div></div>; }
function Nav({ address }: { address?: string | null }) { return <button type="button" onClick={() => openNavigation(address)} disabled={!String(address || '').trim()} className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-50 px-2.5 py-1.5 text-xs font-semibold text-cyan-700 hover:bg-cyan-100 disabled:opacity-50"><Navigation size={13} />Navegar</button>; }
function Row({ order, onOpen }: { order: ServiceOrder; onOpen: (order: ServiceOrder) => void }) { const status = STATUS[order.status] ?? { label: 'Status desconhecido', className: 'bg-slate-100 text-slate-600' }; return <div className="block w-full p-5 hover:bg-slate-50"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => onOpen(order)} className="min-w-0 flex-1 text-left"><div className="flex flex-wrap items-center gap-2"><p className="font-bold">{order.id || 'OS'}</p><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${status.className}`}>{status.label}</span></div><p className="mt-1 font-semibold text-slate-800">{order.client || 'Cliente não informado'}</p><p className="mt-1 text-sm text-slate-500">{order.type || 'Serviço'} · {dateLabel(order.date)}</p><div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500"><MapPin size={13} /><span className="min-w-0">{order.address || 'Endereço não informado'}</span></div></button><div className="flex shrink-0 items-center gap-2 text-sm text-slate-500"><Phone size={15} />{order.phone || '—'}<Nav address={order.address} /></div></div></div>; }
...
export default function TechnicianDashboardV2() {
    ...
    const load = async () => {
      try {
        ...
        try {
          ...
          if (cached.orders.length || cached.user) { setOrders(cached.orders); setUser(cached.user); setError(''); }
          else setError(e instanceof Error ? e.message : 'Não foi possível carregar sua área.');
        } catch (cacheError) { setError(e instanceof Error ? e.message : cacheError instanceof Error ? cacheError.message : 'Não foi possível carregar sua área.'); }
  ...
  const today = useMemo(() => { const key = new Date().toISOString().slice(0, 10); return orders.filter(o => o.date === key || o.status === 'in_progress'); }, [orders]);
  const attendances = useMemo(() => orders.flatMap(order => { const items = Array.isArray(order.attendances) ? order.attendances : []; return items.filter(Boolean).map(attendance => ({ ...attendance, order })); }).sort((a, b) => String(b.startTime || '').localeCompare(String(a.startTime || ''))), [orders]);
