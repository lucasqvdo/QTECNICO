...
export default function TechnicianDashboardV2() {
  ...
  const load = async () => {
    ...
    try {
      const [o, u] = await Promise.all([api.getOrders(), api.getMe()]);
      setOrders(o); setUser(u);
    } catch (e) {
  ...
  const attendances = useMemo(() => orders.flatMap(o => o.attendances.map(a => ({ ...a, order: o }))).sort((a, b) => b.startTime.localeCompare(a.startTime)), [orders]);
  const agenda = useMemo(() => [...orders].filter(o => o.status !== 'cancelled').sort((a, b) => a.date.localeCompare(b.date)), [orders]);

  const content = (() => {
    if (loading) return <div className="p-6 lg:p-8"><div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500">Carregando sua operação...</div></div>;
    if (error) return <div className="p-6 lg:p-8"><div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700">{error}</div></div>;

    if (section === 'home') return <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-7"><p className="text-sm font-medium text-cyan-600">Minha operação</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Olá, {user?.name?.split(' ')[0] || 'Técnico'}!</h1><p className="mt-1 text-sm text-slate-500">Acompanhe suas ordens, atendimentos e programação.</p></div>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat icon={ClipboardIcon} label="Minhas OS" value={orders.length} detail="Ordens atribuídas" />
        <Stat icon={Clock3} label="Em andamento" value={active.length} detail="Pendentes ou em execução" />
        <Stat icon={CheckCircle2} label="Concluídas" value={completed.length} detail="Histórico de execução" />
        <Stat icon={CalendarDays} label="Hoje" value={today.length} detail="Programadas / em execução" />
      </section>
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><h2 className="font-bold">Próximas ordens</h2><p className="text-xs text-slate-500">Chamados sob sua responsabilidade</p></div><div className="divide-y divide-slate-100">{active.slice(0, 6).map(o => <OrderRow key={o.id} order={o} />)}{!active.length && <p className="p-6 text-sm text-slate-500">Nenhuma ordem pendente no momento.</p>}</div></section>
    </div>;

    if (section === 'orders') return <div className="p-4 sm:p-6 lg:p-8"><PageTitle title="Minhas Ordens" subtitle="Chamados atribuídos a você." /><div className="mt-5 space-y-3">{orders.map(o => <OrderRow key={o.id} order={o} />)}{!orders.length && <Empty text="Nenhuma ordem atribuída." />}</div></div>;
  ...
  return <AdminShellV2 section={section} onSectionChange={setSection} mode="technician">{content}</AdminShellV2>;
}

function ClipboardIcon({ size }: { size?: number }) { return <ClipboardListIcon size={size} />; }
function ClipboardListIcon({ size }: { size?: number }) { return <Wrench size={size} />; }
