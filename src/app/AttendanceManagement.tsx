...
import { CalendarClock, Check, Clock3, Edit3, ImagePlus, Plus, Trash2, X } from 'lucide-react';
import { api } from './api';
    ...
    {!orders.length ? <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">Nenhuma ordem de serviço disponível.</div> : <>
      ...
      {order && <div className="grid gap-5 xl:grid-cols-[1.25fr_1fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h2 className="flex items-center gap-2 font-bold"><Clock3 size={18} /> Histórico</h2><p className="mt-1 text-xs text-slate-500">{order.client} · {order.address || 'Endereço não informado'}</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{attendances.length}</span></div>
          {!attendances.length ? <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">Nenhum atendimento registrado. Use “Novo atendimento”.</div> : <div className="space-y-3">{attendances.map(attendance => <article key={attendance.id} className={`rounded-xl border p-4 ${editingId === attendance.id ? 'border-cyan-300 bg-cyan-50/30' : 'border-slate-200'}`}>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h2 className="flex items-center gap-2 font-bold"><CalendarClock size={18} /> {editingId ? 'Editar atendimento' : 'Novo atendimento'}</h2><p className="mt-1 text-xs text-slate-500">O tempo é recalculado automaticamente.</p></div>{(start || end || description || pendingPhotos.length) && <button onClick={resetForm} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Limpar"><X size={18} /></button>}</div>
          <div className="space-y-4"><div><label className="mb-1.5 block text-xs font-semibold text-slate-600">Início</label><input type="datetime-local" value={start} onChange={e => setStart(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm" /></div><div><label className="mb-1.5 block text-xs font-semibold text-slate-600">Término</label><input type="datetime-local" value={end} onChange={e => setEnd(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm" /></div><div><label className="mb-1.5 block text-xs font-semibold text-slate-600">Descrição do atendimento</label><textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} placeholder="O que foi realizado, diagnóstico, observações..." className="w-full resize-none rounded-xl border border-slate-200 px-3 py-3 text-sm" /></div>
            ...
            {start && end && <div className="rounded-xl bg-slate-50 p-3 text-sm"><span className="text-slate-500">Duração calculada: </span><strong>{formatDuration(duration(fromLocalInput(start), fromLocalInput(end)))}</strong></div>}{error && <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700">{error}</div>}
