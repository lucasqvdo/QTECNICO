...
export default function AgendaManagement({ orders, onOrdersChange, onBack }: Props) {
  ...
  return <div className="p-4 sm:p-6 lg:p-8">
    ...
    <div className="mb-5 grid grid-cols-7 gap-2 overflow-x-auto">
      {days.map((day) => { const dayOrders = orders.filter((o) => o.date === day).sort((a,b)=>a.client.localeCompare(b.client,'pt-BR')); const count = dayOrders.length; const active = day === selected; return <button key={day} onClick={() => setSelected(day)} className={`min-w-[130px] rounded-xl border p-3 text-left ${active ? 'border-cyan-400 bg-cyan-50' : 'border-slate-200 bg-white hover:bg-slate-50'}`}><p className="text-[11px] font-semibold uppercase text-slate-500">{new Date(`${day}T12:00:00`).toLocaleDateString('pt-BR', { weekday: 'short' }).replace('.', '')}</p><p className="mt-1 text-xl font-bold">{new Date(`${day}T12:00:00`).getDate()}</p><p className="mt-1 text-[11px] text-slate-500">{count} OS</p>{dayOrders.length > 0 && <div className="mt-2 space-y-1">{dayOrders.slice(0,3).map((o)=><div key={o.id} title={`${o.id} · ${o.client} · ${o.type || 'Serviço'}`} className="rounded-md border border-cyan-100 bg-cyan-50 px-2 py-1 text-[10px] leading-tight text-cyan-800"><span className="block truncate font-semibold">{o.client}</span><span className="block truncate opacity-80">{o.type || o.id}</span></div>)}{dayOrders.length > 3 && <div className="px-1 text-[10px] font-semibold text-slate-500">+{dayOrders.length-3} OS</div>}</div>}</button>; })}
    </div>
