...
export default function SecureLoginScreen({ email, password, error, onEmailChange, onPasswordChange, onLogin, onRegister, onBiometricLogin }: Props) {
  ...
  const handleBiometric = async () => {
    clearFeedback();
    // E-mail é opcional. Vazio inicia o fluxo Passkey discoverable.
  ...
  const handleResetRequest = async () => {
    ...
    try {
      ...
      setMessage(result.message); setResetEmail(target); setMode('code');
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setNewPassword(''); setNewPasswordConfirm(''); setResetCode(''); setMode('login');
      ...
      <div className="relative min-h-screen overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-7xl min-h-screen px-5 py-5 sm:px-8 lg:px-10 lg:py-7 flex flex-col">
          <header className="flex items-center justify-between">
            <div className="flex items-center gap-3"><img src={logoImg} alt="QTECNICO" className="w-10 h-10 object-contain" /><div><div className="text-xl font-extrabold tracking-tight">Q<span className="text-cyan-400">TECNICO</span></div><div className="text-[10px] uppercase tracking-[0.2em] text-white/40">Gestão técnica</div></div></div>
          ...
          {mode === 'login' ? (
            <main className="flex-1 grid lg:grid-cols-[1.15fr_0.85fr] gap-10 xl:gap-20 items-center py-8 lg:py-10">
              <section className="max-w-2xl">
                <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/5 px-3.5 py-2 text-xs font-medium text-cyan-300 mb-5"><span className="w-1.5 h-1.5 rounded-full bg-cyan-400" /> Gestão inteligente para operações técnicas</div>
                <h1 className="text-4xl sm:text-5xl xl:text-6xl font-extrabold leading-[1.04] tracking-tight mb-5">Sua operação técnica <span className="text-cyan-400">sob controle.</span></h1>
                <p className="text-base sm:text-lg leading-7 text-white/55 max-w-xl mb-7">Gerencie ordens de serviço, clientes, técnicos, atendimentos, evidências e financeiro em uma única plataforma criada para quem precisa executar e acompanhar o trabalho de verdade.</p>
                <div className="flex flex-wrap gap-3 mb-8"><button type="button" onClick={() => { clearFeedback(); setMode('register'); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 py-3.5 text-sm font-bold text-slate-950 hover:bg-cyan-300 transition-colors shadow-lg shadow-cyan-400/10">Experimentar o QTECNICO <ArrowRight size={17} /></button><div className="inline-flex items-center rounded-xl border border-white/10 px-4 py-3.5 text-xs text-white/45">Feito para equipes em campo</div></div>
                <div className="grid sm:grid-cols-2 gap-3 max-w-2xl">{featureCards.map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl border border-white/8 bg-white/[0.035] p-4 backdrop-blur-sm"><div className="w-9 h-9 rounded-xl bg-cyan-400/10 flex items-center justify-center mb-3"><Icon size={18} className="text-cyan-400" /></div><h2 className="text-sm font-bold mb-1">{title}</h2><p className="text-xs leading-5 text-white/40">{text}</p></div>)}</div>
              </section>
              <section className="w-full max-w-md lg:justify-self-end"><div className="rounded-3xl bg-white p-6 sm:p-8 shadow-2xl shadow-black/30 text-slate-900 border border-white/10"><div className="mb-6"><div className="text-2xl font-extrabold tracking-tight">Bem-vindo de volta.</div><p className="text-sm text-slate-500 mt-1">Acesse sua conta para continuar.</p></div><LoginForm /></div></section>
            </main>
          ) : (
            <main className="flex-1 flex items-center justify-center py-8"><section className="w-full max-w-md"><div className="rounded-3xl bg-white p-6 sm:p-8 shadow-2xl shadow-black/30 text-slate-900"><div className="flex items-center gap-3 mb-6"><img src={logoImg} alt="QTECNICO" className="w-11 h-11 object-contain" /><div><div className="text-xl font-extrabold">Q<span className="text-cyan-500">TECNICO</span></div><div className="text-xs text-slate-400">Gestão de Ordens de Serviço</div></div></div>{mode === 'register' && <RegisterForm />}{mode === 'request' && <ResetRequestForm />}{mode === 'code' && <CodeForm />}{mode === 'new-password' && <NewPasswordForm />}</div></section></main>
          )}
