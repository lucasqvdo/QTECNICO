...
export function OrderDetail({ order, client, techName, onClose, onUpdate }: {
  ...
  const addExpense = () => {
    ...
    if (editingExpenseId) {
      onUpdate({ ...order, expenses: order.expenses.map(e => e.id === editingExpenseId ? { ...e, label: newExpLabel.trim(), amount } : e) });
    } else {
      onUpdate({ ...order, expenses: [...order.expenses, { id: Date.now().toString(), label: newExpLabel.trim(), amount }] });
    }
    setNewExpLabel(""); setNewExpAmt(""); setEditingExpenseId(null);
  };
  const editExpense = (id: string) => {
    const expense = order.expenses.find(e => e.id === id);
    if (!expense) return;
    setEditingExpenseId(id); setNewExpLabel(expense.label); setNewExpAmt(String(expense.amount));
  };
