...
export default function ConnectivityStatus(){
  ...
  const meta=useMemo(()=>{
    ...
    if(pending>0)return{label:`Online · ${pending} pendente${pending===1?'':'s'}`,Icon:CloudOff,className:'border-amber-200 bg-amber-50 text-amber-700'};
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute right-0 top-11 z-50 w-80 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-xl">
      ...
      <div className="mt-4 space-y-2 text-sm">
        ...
        <div className="flex items-center justify-between"><span className="text-slate-500">Servidor</span><span className={`font-semibold ${!online||state==='offline'||!serverAvailable?'text-red-600':'text-emerald-600'}`}>{!online||state==='offline'?'Sem conexão':serverAvailable?'Disponível':'Indisponível'}</span></div>
