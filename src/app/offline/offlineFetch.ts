import { getAllOffline, getOffline, putOffline, OFFLINE_STORES } from "./offlineDb";
...
async function cacheOrders(orders: any[]) {
  await Promise.all(
    orders
      ...
      .map((order) => putOffline(OFFLINE_STORES.serviceOrders, {
        ...order,
        id: String(order.id),
...
async function handleOfflineOrderUpdate(orderId: string, init?: RequestInit) {
  ...
  return new Response(JSON.stringify(merged), {
    status: 200,
    headers: { "Content-Type": "application/json", "X-QTecnico-Offline": "true" },
  });
}
...
export function installOfflineFetchLayer() {
  window.fetch = async (resource: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (method === "PUT" && orderMatch?.[1]) {
      try {
        return await networkRequest(resource, init);
      } catch (error) {
        if (!isNetworkFailure(error)) throw error;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data)) {
            await Promise.all(data.filter((client) => client?.id != null).map((client) =>
              putOffline(OFFLINE_STORES.clients, { ...client, id: String(client.id), updatedAt: new Date().toISOString() }),
            ));
          }
        }
        return response;
