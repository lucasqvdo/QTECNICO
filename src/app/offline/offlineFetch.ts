...
async function clearPendingOrderSync(id: string, before?: string) {
  const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue);
  for (const item of queue) {
    if (item?.status !== "pending" || item?.entity !== "service_order" || item?.entityId !== id || item?.action !== "update") continue;
    if (before && String(item.createdAt || "") > before) continue;
...
async function queueOrderChange(id: string, action: "update" | "create", payload: any) {
  ...
  if (existing) { await putOffline(OFFLINE_STORES.syncQueue, { ...existing, payload: { ...(existing.payload || {}), ...(payload || {}) }, updatedAt: new Date().toISOString() }); return; }
...
async function handleOfflineOrderUpdate(orderId: string, init?: RequestInit) {
  ...
  if (!patch.operationId) patch.operationId = typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `op-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  if (patch.baseVersion == null && Number.isFinite(Number(cached.syncVersion))) patch.baseVersion = Number(cached.syncVersion);
...
async function handleOfflineAttendancePhoto(orderId: string, attendanceId: string, request: Request) {
  ...
  const attendance = attendances.find((a: any) => String(a.id) === attendanceId); if (!attendance) throw new Error("Atendimento não encontrado no armazenamento offline."); const localPhoto = { id: `photo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, key, dataUrl: photo.dataUrl, name }; attendance.photos.push(localPhoto);
...
async function removePendingPhotoQueues(photo: any) { const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue); for (const item of queue) { const sameTempKey = item?.payload?.tempKey === photo?.key || item?.entityId === photo?.key; const samePhotoId = item?.payload?.photoId === photo?.id || item?.entityId === photo?.id; const uploadForPhoto = item?.entity === "photo_upload" && sameTempKey; const associationForPhoto = item?.entity === "attendance_photo" && samePhotoId; if (uploadForPhoto || associationForPhoto) await deleteOffline(OFFLINE_STORES.syncQueue, item.id); } }
async function handleOfflineAttendancePhotoDelete(orderId: string, attendanceId: string, photoId: string) { const order = await getOffline<any>(OFFLINE_STORES.serviceOrders, orderId); if (!order) throw new Error("Esta OS ainda não foi armazenada no dispositivo."); const attendances = Array.isArray(order.attendances) ? order.attendances.map((a: any) => ({ ...a, photos: Array.isArray(a.photos) ? [...a.photos] : [] })) : []; const attendance = attendances.find((a: any) => String(a.id) === attendanceId); if (!attendance) throw new Error("Atendimento não encontrado no armazenamento offline."); const photo = attendance.photos.find((p: any) => String(p.id) === photoId); if (!photo) throw new Error("Foto não encontrada no armazenamento offline."); const localPhoto = photo.key ? await getOffline<any>(OFFLINE_STORES.photos, photo.key) : undefined; await removePendingPhotoQueues(photo); attendance.photos = attendance.photos.filter((p: any) => String(p.id) !== photoId); await putOffline(OFFLINE_STORES.serviceOrders, { ...order, attendances, offlinePendingSync: true, updatedAt: new Date().toISOString() }); if (localPhoto?.key && String(localPhoto.key).startsWith("offline/")) { await deleteOffline(OFFLINE_STORES.photos, localPhoto.key); return new Response(JSON.stringify({ success: true }), { status: 200, headers: { "Content-Type": "application/json", "X-QTecnico-Offline": "true" } }); } await putOffline(OFFLINE_STORES.syncQueue, { id: `attendance-photo-delete:${orderId}:${attendanceId}:${photoId}`, entity: "attendance_photo_delete", entityId: photoId, action: "delete", payload: { orderId, attendanceId, photoId }, status: "pending", createdAt: new Date().toISOString() }); return new Response(JSON.stringify({ success: true }), { status: 200, headers: { "Content-Type": "application/json", "X-QTecnico-Offline": "true" } }); }
async function reconcileOfflineSignature(tempKey: string, remoteKey: string, remoteUrl: string, localDataUrl?: string) { const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue); const pendingOrders = queue.filter((item) => item?.status === "pending" && item?.entity === "service_order" && item?.action === "update"); for (const item of pendingOrders) { const payload = item.payload && typeof item.payload === "object" ? { ...item.payload } : {}; const matchesKey = payload.clientSignatureKey === tempKey; const matchesUrl = localDataUrl && payload.clientSignature === localDataUrl; if (!matchesKey && !matchesUrl) continue; if (matchesKey) payload.clientSignatureKey = remoteKey; if (matchesUrl || matchesKey) payload.clientSignature = remoteUrl; await putOffline(OFFLINE_STORES.syncQueue, { ...item, payload }); } }
async function flushPhotoQueue() {
  if (!navigator.onLine) return;
  const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue); const uploads = queue.filter((item) => item?.status === "pending" && item?.entity === "photo_upload").sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
  for (const item of uploads) { try { const photo = await getOffline<any>(OFFLINE_STORES.photos, item.entityId); if (!photo?.blob) { await deleteOffline(OFFLINE_STORES.syncQueue, item.id); continue; } const form = new FormData(); form.append("file", photo.blob, photo.name); form.append("folder", photo.folder); const headers: Record<string, string> = {}; const csrf = getCsrfToken(); if (csrf) headers["X-CSRF-Token"] = csrf; const response = await originalFetch(UPLOADS_PATH, { method: "POST", credentials: "include", headers, body: form }); if (!response.ok) break; const saved = await response.json(); await putOffline(OFFLINE_STORES.photos, { ...photo, remoteKey: saved.key, remoteUrl: saved.url, status: "uploaded" }); if (photo.folder === "signatures" && saved.key && saved.url) await reconcileOfflineSignature(photo.key, saved.key, saved.url, photo.dataUrl); await deleteOffline(OFFLINE_STORES.syncQueue, item.id); } catch { break; } }
  const refreshedQueue = await getAllOffline<any>(OFFLINE_STORES.syncQueue); const associations = refreshedQueue.filter((item) => item?.status === "pending" && item?.entity === "attendance_photo").sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
  for (const item of associations) { try { const photo = await getOffline<any>(OFFLINE_STORES.photos, item.payload?.tempKey); if (!photo?.remoteKey) break; const headers: Record<string, string> = { "Content-Type": "application/json" }; const csrf = getCsrfToken(); if (csrf) headers["X-CSRF-Token"] = csrf; const response = await originalFetch(`/api/orders/${encodeURIComponent(item.payload.orderId)}/attendances/${encodeURIComponent(item.payload.attendanceId)}/photos`, { method: "POST", credentials: "include", headers, body: JSON.stringify({ key: photo.remoteKey, url: photo.remoteUrl, name: item.payload.name }) }); if (!response.ok) break; await deleteOffline(OFFLINE_STORES.syncQueue, item.id); const orderResponse = await originalFetch(`/api/orders/${encodeURIComponent(item.payload.orderId)}`, { credentials: "include" }); if (orderResponse.ok) { const fresh = await orderResponse.json().catch(() => null); if (fresh?.id != null) await cacheOrder(fresh); } await deleteOffline(OFFLINE_STORES.photos, item.payload.tempKey); } catch { break; } }
  ...
  for (const item of deleteQueue) { try { const headers: Record<string, string> = {}; const csrf = getCsrfToken(); if (csrf) headers["X-CSRF-Token"] = csrf; const response = await originalFetch(`/api/orders/${encodeURIComponent(item.payload.orderId)}/attendances/${encodeURIComponent(item.payload.attendanceId)}/photos/${encodeURIComponent(item.payload.photoId)}`, { method: "DELETE", credentials: "include", headers }); if (!response.ok) break; await deleteOffline(OFFLINE_STORES.syncQueue, item.id); const orderResponse = await originalFetch(`/api/orders/${encodeURIComponent(item.payload.orderId)}`, { credentials: "include" }); if (orderResponse.ok) { const fresh = await orderResponse.json().catch(() => null); if (fresh?.id != null) await cacheOrder(fresh); } } catch { break; } }
}
async function flushOrderQueue() {
  if (!navigator.onLine) return; await flushPhotoQueue();
  const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue); const pending = queue.filter((item) => (item?.status === "pending" || item?.status === "conflict") && item?.entity === "service_order" && item?.entityId && item?.action === "update").sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
  for (const item of pending) {
    if (item?.status === "conflict") continue;
    if (activeOrderMutations.has(String(item.entityId))) continue;
    try {
      ...
      const response = await originalFetch(`/api/orders/${encodeURIComponent(String(item.entityId))}/sync`, { method: "PUT', credentials: "include', headers, body: JSON.stringify(item.payload ?? {}) });
      const saved = await response.clone().json().catch(() => null);
      if (response.status === 409) { await putOffline(OFFLINE_STORES.syncQueue, { ...item, status: "conflict", conflict: saved, updatedAt: new Date().toISOString() }); break; }
...
export function installOfflineFetchLayer() {
  window.fetch = async (resource: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (method === "GET" && orderMatch) { try { const response = await networkRequest(resource, init); if (response.ok) { const data = await response.clone().json().catch(() => null); if (Array.isArray(data)) await cacheOrders(data); else if (data?.id != null && orderMatch[1]) { const id = decodeURIComponent(orderMatch[1]); if (await hasPendingOrderSync(id)) return handleOfflineOrderDetail(id); await cacheOrder(data); } } return response; } catch (error) { if (!isNetworkFailure(error)) throw error; if (orderMatch[1]) return handleOfflineOrderDetail(decodeURIComponent(orderMatch[1])); return handleOfflineOrderList(); } }
    if (method === "PUT" && orderMatch?.[1]) { const orderId = decodeURIComponent(orderMatch[1]); const mutationStartedAt = new Date().toISOString(); activeOrderMutations.add(orderId); try { const response = await networkRequest(resource, init); if (response.ok) { await clearPendingOrderSync(orderId, mutationStartedAt); const saved = await response.clone().json().catch(() => null); if (saved?.id != null) await cacheOrder({ ...saved, offlinePendingSync: false }); } return response; } catch (error) { if (!isNetworkFailure(error)) throw error; return handleOfflineOrderUpdate(orderId, init); } finally { activeOrderMutations.delete(orderId); } }
