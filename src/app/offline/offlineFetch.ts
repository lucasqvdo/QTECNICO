...
async function queueOrderChange(id: string, action: "update" | "create", payload: any) { const queueId = `${action}:order:${id}:${Date.now()}:${Math.random().toString(36).slice(2, 8)}`; await putOffline(OFFLINE_STORES.syncQueue, { id: queueId, entity: "service_order", entityId: id, action, payload, status: "pending", createdAt: new Date().toISOString() }); }
...
export function installOfflineFetchLayer() {
  window.fetch = async (resource: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const orderMatch = ORDER_PATH.exec(url.pathname); const attendancePhotoMatch = ATTENDANCE_PHOTO_PATH.exec(url.pathname);
