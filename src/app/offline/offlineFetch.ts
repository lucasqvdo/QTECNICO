...
async function removePendingPhotoQueues(photo: any) {
  const queue = await getAllOffline<any>(OFFLINE_STORES.syncQueue);
  for (const item of queue) { const sameTempKey = item?.payload?.tempKey === photo?.key || item?.entityId === photo?.key; const samePhotoId = item?.payload?.photoId === photo?.id || item?.entityId === photo?.id; const uploadForPhoto = item?.entity === "photo_upload" && sameTempKey; const associationForPhoto = item?.entity === "attendance_photo" && samePhotoId; if (uploadForPhoto || associationForPhoto) await deleteOffline(OFFLINE_STORES.syncQueue, item.id); }
}
async function handleOfflineAttendancePhotoDelete(orderId: string, attendanceId: string, photoId: string) {
  const order = await getOffline<any>(OFFLINE_STORES.serviceOrders, orderId); if (!order) throw new Error("Esta OS ainda não foi armazenada no dispositivo."); const attendances = Array.isArray(order.attendances) ? order.attendances.map((a: any) => ({ ...a, photos: Array.isArray(a.photos) ? [...a.photos] : [] })) : []; const attendance = attendances.find((a: any) => String(a.id) === attendanceId); if (!attendance) throw new Error("Atendimento não encontrado no armazenamento offline."); const photo = attendance.photos.find((p: any) => String(p.id) === photoId); if (!photo) throw new Error("Foto não encontrada no armazenamento offline."); const localPhoto = photo.key ? await getOffline<any>(OFFLINE_STORES.photos, photo.key) : undefined;
  await removePendingPhotoQueues(photo); attendance.photos = attendance.photos.filter((p: any) => String(p.id) !== photoId); await putOffline(OFFLINE_STORES.serviceOrders, { ...order, attendances, offlinePendingSync: true, updatedAt: new Date().toISOString() });
  if (localPhoto?.key && String(localPhoto.key).startsWith("offline/")) { await deleteOffline(OFFLINE_STORES.photos, localPhoto.key); return new Response(JSON.stringify({ success: true }), { status: 200, headers: { "Content-Type": "application/json", "X-QTecnico-Offline": "true" } }); }
  await putOffline(OFFLINE_STORES.syncQueue, { id: `attendance-photo-delete:${orderId}:${attendanceId}:${photoId}`, entity: "attendance_photo_delete", entityId: photoId, action: "delete", payload: { orderId, attendanceId, photoId }, status: "pending", createdAt: new Date().toISOString() });
...
async function flushOrderQueue() {
  ...
  for (const item of pending) {
    ...
    try { const headers: Record<string, string> = { "Content-Type": "application/json" }; const csrf = getCsrfToken(); if (csrf) headers["X-CSRF-Token"] = csrf; const response = await originalFetch(`/api/orders/${encodeURIComponent(String(item.entityId))}`, { method: "PUT", credentials: "include", headers, body: JSON.stringify(item.payload ?? {}) }); if (!response.ok) break; const saved = await response.clone().json().catch(() => null); if (saved?.id != null) await cacheOrder({ ...saved, offlinePendingSync: false }); await deleteOffline(OFFLINE_STORES.syncQueue, item.id); } catch { break; }
  }
}
export function installOfflineFetchLayer() {
  window.fetch = async (resource: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (method === "GET" && orderMatch) {
      try { const response = await networkRequest(resource, init); if (response.ok) { const data = await response.clone().json().catch(() => null); if (Array.isArray(data)) await cacheOrders(data); else if (data?.id != null && orderMatch[1]) { const id = decodeURIComponent(orderMatch[1]); if (await hasPendingOrderSync(id)) return handleOfflineOrderDetail(id); await cacheOrder(data); } } return response; } catch (error) { if (!isNetworkFailure(error)) throw error; if (orderMatch[1]) return handleOfflineOrderDetail(decodeURIComponent(orderMatch[1])); return handleOfflineOrderList(); }
    }
    if (method === "PUT" && orderMatch?.[1]) {
      ...
      try {
        ...
        if (response.ok) {
          // A successful online edit is newer than any queue entry created before it.
          // Remove only older pending mutations, preserving a genuinely newer offline edit.
          await clearPendingOrderSync(orderId, mutationStartedAt);
        ...
        return handleOfflineOrderUpdate(orderId, init);
      } finally {
