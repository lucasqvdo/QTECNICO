...
export default function FinanceManagement({ orders, onOrdersChange, onBack }: { orders: ServiceOrder[]; onOrdersChange: (orders: ServiceOrder[]) => void; onBack: () => void }) {
  ...
  const [companyExpenseForm, setCompanyExpenseForm] = useState({description:'',category:'Outros',supplier:'',amount:'',dueDate:today(),status:'pending' as 'pending'|'paid',recurrence:'once' as 'once'|'monthly'|'quarterly'|'yearly',paymentMode:'single' as 'single'|'installments',installmentCount:'2',paymentMethod:'other',notes:''});
  ...
  const openCompanyExpense=(expense?:CompanyExpense)=>{setCompanyExpenseEditing(expense||null);setCompanyExpenseForm(expense?{description:expense.description,category:expense.category,supplier:expense.supplier,amount:String(expense.amount),dueDate:String(expense.dueDate).slice(0,10),status:expense.status,recurrence:expense.recurrence,paymentMode:expense.installmentCount>1?'installments':'single',installmentCount:String(expense.installmentCount||2),notes:expense.notes}:{description:'',category:'Outros',supplier:'',amount:'',dueDate:today(),status:'pending',recurrence:'once',paymentMode:'single',installmentCount:'2',notes:''});setCompanyExpenseModal(true)};
  const saveCompanyExpense=async()=>{const installments=companyExpenseForm.paymentMode==='installments'?Math.floor(Number(companyExpenseForm.installmentCount)):1;if(!companyExpenseForm.description.trim()||Number(companyExpenseForm.amount)<=0||!companyExpenseForm.dueDate){setError('Informe descrição, valor e vencimento.');return}if(companyExpenseForm.paymentMode==='installments'&&(installments<2||installments>120)){setError('Informe uma quantidade de parcelas entre 2 e 120.');return}setSaving(true);setError('');try{const {paymentMode,installmentCount:_,...form}=companyExpenseForm;const payload={...form,amount:Number(form.amount),paidAt:form.status==='paid'?form.dueDate:null,recurrence:paymentMode==='installments'?'once' as const:form.recurrence,installmentCount:installments,paymentMethod:companyExpenseForm.paymentMethod};if(companyExpenseEditing){const saved=await api.updateCompanyExpense(companyExpenseEditing.id,payload);setCompanyExpenses(x=>x.map(e=>e.id===saved.id?saved:e))}else{const saved=await api.createCompanyExpense(payload);setCompanyExpenses(x=>[...saved,...x])}setCompanyExpenseModal(false)}catch(e){setError(e instanceof Error?e.message:'Não foi possível salvar a despesa.')}finally{setSaving(false)}};
  ...
  return <div className="p-4 sm:p-6 lg:p-8">
    ...
    {companyExpenseModal && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 sm:items-center sm:p-4"><div className="flex max-h-[92dvh] w-full flex-col rounded-t-2xl bg-white shadow-xl sm:max-w-lg sm:rounded-2xl"><div className="flex shrink-0 items-center justify-between border-b px-4 py-4 sm:p-5"><h2 className="font-bold">{companyExpenseEditing?'Editar despesa':'Nova despesa da empresa'}</h2><button onClick={()=>setCompanyExpenseModal(false)} className="rounded-lg p-2 hover:bg-slate-100" aria-label="Fechar"><X size={20}/></button></div><div className="grid flex-1 gap-4 overflow-y-auto overscroll-contain px-4 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:grid-cols-2 sm:p-5"><label className="text-sm font-medium sm:col-span-2">Descrição<input value={companyExpenseForm.description} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,description:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"/></label><label className="text-sm font-medium">Categoria<select value={companyExpenseForm.category} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,category:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"><option>Aluguel</option><option>Internet / Telefonia</option><option>Combustível</option><option>Ferramentas / Equipamentos</option><option>Contabilidade</option><option>Software / Assinaturas</option><option>Impostos</option><option>Salários / Pró-labore</option><option>Outros</option></select></label><label className="text-sm font-medium">Fornecedor<input value={companyExpenseForm.supplier} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,supplier:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"/></label><label className="text-sm font-medium">Valor<input type="number" min="0.01" step="0.01" value={companyExpenseForm.amount} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,amount:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"/></label><label className="text-sm font-medium">Vencimento<input type="date" value={companyExpenseForm.dueDate} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,dueDate:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"/></label><label className="text-sm font-medium">Status<select value={companyExpenseForm.status} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,status:e.target.value as 'pending'|'paid'})} className="mt-1 w-full rounded-xl border px-3 py-2"><option value="pending">Pendente</option><option value="paid">Pago</option></select></label><label className="text-sm font-medium">Forma de pagamento<select value={companyExpenseForm.paymentMethod} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,paymentMethod:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"><option value="pix">PIX</option><option value="cash">Dinheiro</option><option value="credit_card">Cartão de crédito</option><option value="debit_card">Cartão de débito</option><option value="boleto">Boleto</option><option value="bank_transfer">Transferência bancária</option><option value="automatic_debit">Débito automático</option><option value="check">Cheque</option><option value="other">Outro</option></select></label><label className="text-sm font-medium">Pagamento<select value={companyExpenseForm.paymentMode} disabled={!!companyExpenseEditing} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,paymentMode:e.target.value as 'single'|'installments'})} className="mt-1 w-full rounded-xl border px-3 py-2 disabled:bg-slate-100"><option value="single">À vista / único</option><option value="installments">Parcelado</option></select></label>{companyExpenseForm.paymentMode==='installments'&&!companyExpenseEditing&&<label className="text-sm font-medium">Quantidade de parcelas<input type="number" min="2" max="120" step="1" value={companyExpenseForm.installmentCount} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,installmentCount:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2"/><span className="mt-1 block text-xs text-slate-500">O valor total será dividido e os vencimentos serão mensais.</span></label>}{companyExpenseForm.paymentMode!=='installments'&&<label className="text-sm font-medium">Recorrência<select value={companyExpenseForm.recurrence} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,recurrence:e.target.value as any})} className="mt-1 w-full rounded-xl border px-3 py-2"><option value="once">Única</option><option value="monthly">Mensal</option><option value="quarterly">Trimestral</option><option value="yearly">Anual</option></select></label>}<label className="text-sm font-medium sm:col-span-2">Observações<textarea value={companyExpenseForm.notes} onChange={e=>setCompanyExpenseForm({...companyExpenseForm,notes:e.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2" rows={3}/></label><div className="sticky bottom-0 -mx-4 -mb-4 flex flex-col-reverse gap-2 border-t bg-white px-4 py-3 sm:static sm:mx-0 sm:mb-0 sm:col-span-2 sm:flex-row sm:justify-end sm:border-0 sm:bg-transparent sm:p-0"><button onClick={()=>setCompanyExpenseModal(false)} className="w-full rounded-xl border px-4 py-3 text-sm font-semibold sm:w-auto sm:py-2">Cancelar</button><button disabled={saving} onClick={()=>void saveCompanyExpense()} className="w-full rounded-xl bg-cyan-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50 sm:w-auto sm:py-2">{saving?'Salvando...':'Salvar despesa'}</button></div></div></div></div>}
