...
export default function FinanceManagement({ orders, onOrdersChange, onBack }: { orders: ServiceOrder[]; onOrdersChange: (orders: ServiceOrder[]) => void; onBack: () => void }) {
  ...
  const [companyExpenseForm, setCompanyExpenseForm] = useState({description:'',category:'Outros',supplier:'',amount:'',dueDate:today(),status:'pending' as 'pending'|'paid',recurrence:'once' as 'once'|'monthly'|'quarterly'|'yearly',paymentMode:'single' as 'single'|'installments',installmentCount:'2',notes:''});
