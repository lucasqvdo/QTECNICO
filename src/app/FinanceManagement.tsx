...
import { ArrowDownCircle, ArrowUpCircle, DollarSign, Plus, Trash2, X } from 'lucide-react';
import { api } from './api';
...
export default function FinanceManagement({ orders, onOrdersChange, onBack }: { orders: ServiceOrder[]; onOrdersChange: (orders: ServiceOrder[]) => void; onBack: () => void }) {
  ...
  const openModal = (kind: Modal, order: ServiceOrder) => {
    setModal(kind); setSelectedOrder(order); setLabel(kind === 'payment' ? 'Pagamento' : ''); setAmount(''); setDate(today()); setStatus('paid'); setError('');
  };
  ...
  const saveFinance = async () => {
    ...
    try {
      ...
      if (modal === 'payment') {
        const payment: Payment = { id: `pay-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, orderId: selectedOrder.id, label: label.trim(), amount: value, date, status };
        const payments = [...selectedOrder.payments, payment];
        ...
        const expense: Expense = { id: `exp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, label: label.trim(), amount: value };
        next = { ...selectedOrder, expenses: [...selectedOrder.expenses, expense] };
      }
      const saved = await api.updateOrder(selectedOrder.id, next);
      onOrdersChange(orders.map((o) => o.id === saved.id ? saved : o));
      setModal(null); setSelectedOrder(null);
