...
export default function ClientsManagement({ clients, orders, onClientsChange, onBack }: Props) {
  ...
  const openNew = () => { setError(''); setEditing(null); setForm(emptyForm); setSelectedId(null); };
  const openEdit = (client: Client) => { setError(''); setEditing(client); setForm({ name: client.name, document: client.document, address: client.address, phone: client.phone, email: client.email }); setSelectedId(null); };
  ...
  return <div className="p-4 sm:p-6 lg:p-8">
    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      ...
      <button onClick={openNew} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-slate-800"><Plus size={18} /> Novo cliente</button>
    </div>
