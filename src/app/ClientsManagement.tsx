...
export default function ClientsManagement({ clients, orders, onClientsChange, onBack }: Props) {
  ...
  const selectClient = async (client: Client) => { setSelectedId(client.id); setPortal(null); setPortalPassword(''); setPortalPhone(client.phone || ''); try { const access = await api.getClientPortalAccess(client.id); setPortal(access); if (access?.phone) setPortalPhone(access.phone); } catch (e) { setError(e instanceof Error ? e.message : 'Não foi possível carregar o portal do cliente.'); } };
  ...
  const openEdit = (client: Client) => { setError(''); setCreating(false); setEditing(client); setForm({ name: client.name, document: client.document, address: client.address, phone: client.phone, email: client.email, recurringMaintenanceEnabled: Boolean(client.recurringMaintenanceEnabled) }); setSelectedId(null); };
