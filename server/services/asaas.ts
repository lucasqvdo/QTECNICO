...
function getBaseUrl(): string {
  ...
  // Never allow a production environment to accidentally point at Sandbox (or vice versa).
  if (configured) {
    if (environment === 'sandbox' && !configured.startsWith(DEFAULT_SANDBOX_URL)) {
      throw new Error('ASAAS_API_URL incompatível com PAYMENT_ENV=sandbox.');
    }
    if (environment === 'production' && !configured.startsWith(DEFAULT_PRODUCTION_URL)) {
      throw new Error('ASAAS_API_URL incompatível com PAYMENT_ENV=production.');
    }
    return configured;
  }
...
function getApiKey(): string {
  ...
  const environment = getEnvironment();
  if (environment === 'sandbox' && !key.startsWith('$aact_hmlg_')) {
    throw new Error('ASAAS_API_KEY não parece ser uma chave Sandbox.');
  }
  if (environment === 'production' && !key.startsWith('$aact_prod_')) {
    throw new Error('ASAAS_API_KEY não parece ser uma chave de Produção.');
  }
...
export async function getAsaasStatus() {
  const environment = getEnvironment();
  const accountStatus = await asaasRequest<{
