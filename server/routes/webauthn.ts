...
router.post('/register/verify', webAuthnRateLimit, requireAuth, async (req, res) => {
  try {
    ...
    const challengeResult = await pool.query(
      `SELECT challenge FROM webauthn_challenges WHERE user_id = $1 AND type = 'registration' AND expires_at > NOW() ORDER BY expires_at DESC LIMIT 1`,
      [userId],
    );
...
router.post('/authenticate/verify', webAuthnRateLimit, async (req, res) => {
  try {
    ...
    const challengeResult = await pool.query(
      `SELECT challenge FROM webauthn_challenges WHERE user_id = $1 AND type = 'authentication' AND expires_at > NOW() ORDER BY expires_at DESC LIMIT 1`,
    ...
    await pool.query('UPDATE webauthn_credentials SET counter = $1 WHERE id = $2 AND counter = $3', [newCounter, credentialRow.id, credentialRow.counter]);
