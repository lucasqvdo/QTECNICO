...
router.get('/profile', requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, legal_name, trade_name, document, phone, email, website,
              address, number, complement, neighborhood, city, state, postal_code,
              logo_url, description, updated_at
       FROM company_profiles WHERE account_id = $1`,
      [accountId]
    );
...
router.put('/profile', requireAdmin, async (req, res) => {
  ...
  const data = {
    legalName: normalize(req.body?.legalName),
    tradeName: normalize(req.body?.tradeName),
    ...
    number: normalize(req.body?.number),
    complement: normalize(req.body?.complement),
    neighborhood: normalize(req.body?.neighborhood),
    city: normalize(req.body?.city),
    state: normalize(req.body?.state).toUpperCase().slice(0, 2),
    postalCode: normalize(req.body?.postalCode),
    logoUrl: normalize(req.body?.logoUrl),
    description: normalize(req.body?.description),
  };

  if (!data.tradeName && !data.legalName) {
    return res.status(400).json({ error: 'Informe pelo menos a razão social ou o nome fantasia' });
  }

  try {
    const result = await pool.query(
      `INSERT INTO company_profiles
        (account_id, legal_name, trade_name, document, phone, email, website,
         address, number, complement, neighborhood, city, state, postal_code,
         logo_url, description, updated_at)
       ...
       ON CONFLICT (account_id) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING id, legal_name, trade_name, document, phone, email, website,
                 address, number, complement, neighborhood, city, state, postal_code,
                 logo_url, description, updated_at`,
      [accountId, data.legalName, data.tradeName, data.document, data.phone, data.email,
       data.website, data.address, data.number, data.complement, data.neighborhood,
       data.city, data.state, data.postalCode, data.logoUrl, data.description]
    );

    const c = result.rows[0];
    res.json({
      id: c.id,
      legalName: c.legal_name || '', tradeName: c.trade_name || '', document: c.document || '',
      ...
      city: c.city || '', state: c.state || '', postalCode: c.postal_code || '',
      logoUrl: c.logo_url || '', description: c.description || '', updatedAt: c.updated_at,
    });
  } catch (error) {
