...
async function getAccountContext(userId: number) {
  const result = await pool.query(
    `SELECT a.id AS account_id, a.owner_user_id, a.plan_key, a.subscription_status, a.current_period_end,
            cp.trade_name, cp.legal_name, cp.document,
            cp.email AS company_email, cp.phone, u.name AS owner_name, u.email AS owner_email
       FROM users current_u
       JOIN accounts a ON a.id = current_u.account_id OR a.owner_user_id = current_u.id
       LEFT JOIN company_profiles cp ON cp.account_id = a.id
       LEFT JOIN users u ON u.id = a.owner_user_id
      WHERE current_u.id = $1 LIMIT 1`,
    [userId],
  );
  ...
  try {
    ...
    return res.json({
      ...
      account: {
        ...
        company: account.trade_name || account.legal_name || 'Empresa',
        email: account.company_email || account.owner_email || '',
