...
router.get('/summary', requireAdmin, async (req, res) => {
  try {
    ...
    const [stats, monthly, technicians, clients, recent] = await Promise.all([
      pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE o.status='pending')::int AS pending, COUNT(*) FILTER (WHERE o.status='in_progress')::int AS in_progress, COUNT(*) FILTER (WHERE o.status='completed')::int AS completed, COUNT(*) FILTER (WHERE o.status='cancelled')::int AS cancelled, COUNT(*) FILTER (WHERE o.date = CURRENT_DATE)::int AS today, COALESCE(SUM(o.client_value),0)::numeric AS revenue, COALESCE((SELECT SUM(p.amount) FROM order_payments p WHERE p.account_id=$1 AND p.status='paid' ${startDate ? 'AND p.date >= $2' : ''}),0)::numeric AS paid, COALESCE((SELECT SUM(e.amount) FROM expenses e WHERE e.account_id=$1 ${startDate ? 'AND (e.order_id IS NULL OR EXISTS (SELECT 1 FROM orders eo WHERE eo.id=e.order_id AND eo.created_at >= $2 AND eo.account_id=$1))' : ''}),0)::numeric AS costs FROM orders o WHERE o.account_id=$1 ${dateClause}`, dateParams),
    ...
    res.json({
      ...
      stats:{
        total, pending:Number(row.pending||0), inProgress:Number(row.in_progress||0), completed:Number(row.completed||0), cancelled:Number(row.cancelled||0), today:Number(row.today||0),
