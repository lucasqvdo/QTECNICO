...
router.put('/:id/sync', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (operationId) {
      ...
      if (op.rows[0]?.status === 'completed') { await db.query('ROLLBACK'); return res.status(op.rows[0].response_status || 200).json(op.rows[0].response_body || { error: 'Operação já processada' }); }
      if (op.rows[0]?.status === 'processing') { await db.query('ROLLBACK'); return res.status(409).json({ error: 'Operação de sincronização ainda está sendo processada', code: 'SYNC_OPERATION_PROCESSING', operationId }); }
      if (!op.rows[0]) await db.query(`INSERT INTO sync_operations (operation_id,account_id,entity_type,entity_id,base_version,status) VALUES ($1,$2,'service_order',$3,$4,'processing')`, [operationId, accountId, id, Number.isFinite(baseVersion) ? baseVersion : null]);
    }
    ...
    if (!existing) { await db.query('ROLLBACK'); return res.status(404).json({ error: 'Ordem não encontrada' }); }
    ...
    if (!isAdmin && body.assignedTechnicianIds) { await db.query('ROLLBACK'); return res.status(403).json({ error: 'Técnico não pode alterar a atribuição da O.S.' }); }
    ...
    if (has('clientSignatureKey') && signatureKey != null && !isSafeStorageKey(signatureKey, accountId, 'signatures')) { await db.query('ROLLBACK'); return res.status(400).json({ error: 'Assinatura inválida para esta conta' }); }

    if (has('attendances')) {
      const ctx = await getAccountContext(userId);
      if (ctx) assertPhotoLimit(ctx.plan, body.attendances || []);
      for (const a of (body.attendances || [])) for (const p of (a.photos || [])) if (p.key && !isSafeStorageKey(p.key, accountId, 'attendances')) { await db.query('ROLLBACK'); return res.status(400).json({ error: 'Foto de atendimento inválida para esta conta' }); }
      ...
      for (const a of (body.attendances || [])) {
        ...
        for (const p of (a.photos || [])) if (p.key) await db.query('INSERT INTO attendance_photos (id,account_id,attendance_id,data_url,name) VALUES ($1,$2,$3,$4,$5)', [p.id || `${Date.now()}-${Math.random()}`, accountId, attendanceId, p.key, p.name || '']);
      }
    }
    ...
    if (operationId && accountId) { try { await pool.query(`UPDATE sync_operations SET status='failed',response_status=500,response_body=$1::jsonb,processed_at=now() WHERE operation_id=$2 AND account_id=$3`, [JSON.stringify({ error: e?.message || 'Erro ao sincronizar O.S.' }), operationId, accountId]); } catch {} }
