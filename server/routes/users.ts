...
async function getCompanyProfile(accountId: number) {
  ...
  if (!c) return null;
  return { id: c.id, legalName: c.legal_name || '', tradeName: c.trade_name || '', document: c.document || '', phone: c.phone || '', whatsapp: c.whatsapp || '', email: c.email || '', website: c.website || '', address: c.address || '', number: c.number || '', complement: c.complement || '', neighborhood: c.neighborhood || '', city: c.city || '', state: c.state || '', postalCode: c.postal_code || '', logoKey: c.logo_key || '', description: c.description || '', updatedAt: c.updated_at };
}
...
router.put('/admin/company-profile', requireAdmin, async (req, res) => {
  try {
    ...
    const data = {
      ...
      state: normalize(req.body?.state).toUpperCase().slice(0, 2), postalCode: normalize(req.body?.postalCode), logoKey: normalize(req.body?.logoKey), description: normalize(req.body?.description),
    };
